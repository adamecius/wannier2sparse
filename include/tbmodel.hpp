...
class tbmodel
{
    ...
    // is expanded/exported by the same wrap/save path. Enables hand-built TB
    // models (Haldane, Kane-Mele, BHZ) and externally generated operators.
    ...
    {
        // Current operators are generated by multiplying each hopping by the
    ...
    {
        // Density is local in the unit cell: keep onsite terms and zero all
        // translated hoppings while preserving the same sparse structure.
        assert(volume(lat_vecs) > 0 );
    ...
    {
        // Spin-density operators reuse the density support and replace the
        // values by the requested Pauli matrix in the spin-resolved basis.
    ...
    {
        // Spin currents are current operators projected through the selected
        // spin Pauli matrix in the same spin-resolved orbital ordering.
